...
Blob::Blob(const Particles &particles)
  ...
  , simpleShader(("simple.vert", "simple.frag"))
...
// 図形の描画
void Blob::drawObject(const GgMatrix &mp, const GgMatrix &mv,
  ...
  const GgApplication::Window &window) const
{
  ...
  //図形の描画
  //矩形を描くシェーダを選択する
  simpleShader.use(mp, mv * window.getTrackball(1), mv.normal(), light);
  object.draw();
